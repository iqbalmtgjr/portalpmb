...
class Mbank extends CI_Model
{
    ...
    public function item_bayarsemua($id)
    {
        ...
        $this->db->join('biaya_kuliah_pmbtest', 'jenis_bayar_rinci = id_biayakuliahpmb', 'left');
    ...
    public function percoxxx($names)
    {
        ...
        $data = $this->db->get('biaya_kuliah_pmbtest');
        return $data->result();
    }
    ...
    {
        $this->db->where('id_biayakuliahpmb', $id);
        $result = $this->db->get('biaya_kuliah_pmbtest');
        if ($this->db->affected_rows()) {
            return $result;
        } else {
            return false;
    ...
    {
        $alamat = base_url();
        $this->datatables->select('id_siswa,akun_siswa,nama_siswa,ujian,pilihan_satu,pilihan_dua,jalur,nama_prodi,nama_prodi_baru,pembayaran_upload,valid_bayar,keputusan_text,metode_bayar,umumkan');
    ...
    {
        $alamat = base_url();
        $this->datatables->select('id_siswa,akun_siswa,nama_siswa,ujian,pilihan_satu,pilihan_dua,jalur,nama_prodi,nama_prodi_baru,pembayaran_upload,valid_bayar,keputusan_text,metode_bayar,nama_prodip,umumkan,status_penerimaan');
        $this->datatables->from('pmb_siswa');
        $this->datatables->join('pmb_akun', 'akun_siswa = pengenal_akun', 'left');
        $this->datatables->join('pmb_upload', 'akun_siswa = upload_id_siswa', 'left');
        $this->datatables->join('pmb_prodi', 'akun_siswa = prodi_id_siswa', 'left');
    ...
    public function itemBiayabarutest()
    {
        $data = $this->db->get('biaya_kuliah_pmbtest');
        return $data->result();
    }
    ...
    public function cobaCobatest($item)
    {
        $this->db->where_in('id_biayakuliahpmb', $item);
        return $this->db->get('biaya_kuliah_pmbtest')->result();
    }

...
class Bank extends CI_Controller
{
	...
	{
		if ($this->session->userdata('pengguna_id_simkeu') == "45" || $this->session->userdata('pengguna_id_simkeu') == "49" || $this->session->userdata('pengguna_id_simkeu') == "48" || $this->session->userdata('pengguna_id_simkeu') == "52") {
			$data = array(
			...
			if (empty($ss)) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if ($dutu->jalur == "prestasi") {
						...
						$billitem = $this->bank->itemBiayabarutest();
					}
					$data = array(
						'bank' => 'lihat',
						'pmb' => "",
		...
		if ($this->session->userdata('pengguna_id_simkeu') == "45" || $this->session->userdata('pengguna_id_simkeu') == "49") {
			...
			if (!empty($ss)) {
				if ($ss->validasi_bukti !== 2) {
					...
					$res = $this->db->delete('pembayaran_rinci', $where1);
					if ($res >= 1) {
						$this->session->set_flashdata('pesan', '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h5><i class="icon fas fa-ban"></i> Alert!</h5>Simpan Data Berhasil.</div>');
						redirect('bank/index');
					} else {
			...
			if (!empty($cek)) {
				...
				if (!empty($dum)) {
					...
					if ($dd >= 1) {
						...
						$message .= 'Kami menyampaikan informasi mengenai pelaksanaan tes tertulis dan tes wawancara.<br>';
						$message .= 'Tes tertulis PMB dilaksanakan secara online, dimulai pada tanggal 1 sampai 15 Mei 2023.<br>';
						$message .= 'Tes Wawancara PMB dilakukan melalui telpon/WhatsApp.<br><br>';
						$message .= '<strong>Keterangan Tes Tertulis: </strong><br>';
						$message .= 'Silahkan masuk ke ' . $link . ' untuk melakukan Tes Calon Mahasiswa Baru STKIP Persada Khatulistiwa Tahun 2023';
						$message .= '<strong> Link : </strong>' . $url . '<br>';
						$message .= 'Gunakan email : ' . $cek->email_akun_siswa . ' dan password: <strong> ' . $cek->kuncigudang . '</strong><br>';
						$message .= 'Calon mahasiswa bisa memilih sendiri waktu tes selama tes masih dibuka (1 - 15 Mei 2023)<br>';

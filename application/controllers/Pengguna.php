...
//		$this->template->view('user/dispen', $data);
 ...
 //   }
    ...
    public function tambah(){
		...
		$this->form_validation->set_rules('mail', 'Email','trim|required|is_unique[pengguna.email]',
			array(
					'required'      => '%s tidak boleh kosong!',
					'is_unique'      => '%s sudah digunakan!'
			)
        );
		$this->form_validation->set_rules('kunci', 'Password','trim|required|min_length[8]',
			array(
					'required'      => '%s tidak boleh kosong!',
			)
        );
        ...
        $this->form_validation->set_rules('apli', 'Aplikasi','trim|required',
			array(
					'required'      => '%s tidak boleh kosong!',
			)
        );
     ...
     public function ubah($id){
         if($this->session->userdata('pengguna_id_simkeu') == "45" || $this->session->userdata('pengguna_id_simkeu') == "46"){
    		...
    		$this->form_validation->set_rules('nma', 'Nama Pengguna','trim|required',
    			array(
    					'required'      => '%s tidak boleh kosong!',					
    			)
            );
    ...
    public function ganti(){
        ...
        $this->form_validation->set_rules('baru', 'Password Baru', 'trim|required|min_length[8]|max_length[12]',
        array(
		...
		$this->form_validation->set_rules('ulang', 'Pengulangan Password', 'trim|required|matches[baru]',
			array(
	...
	public function foto_do() {
		...
		if(empty($data)){
			...
			$config['upload_path'] = '../simkeu/foto/';
